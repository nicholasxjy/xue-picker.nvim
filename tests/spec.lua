...
test("smart keeps displayed rows and selection until the replacement search finishes", function()
  ...
  s:act("toggle_all")
  await(function()
    return not s.render_timer and #api.nvim_buf_get_lines(s.ui.bufs.preview, 0, -1, false) > 1
  end)
  local lines = api.nvim_buf_get_lines(s.ui.bufs.list, 0, -1, false)
  local preview = api.nvim_buf_get_lines(s.ui.bufs.preview, 0, -1, false)
  local frames, render = {}, s.ui.render
  s.ui.render = function(ui)
    render(ui)
    frames[#frames + 1] = #s.results
  end
  s:set_query("slow")
  s.ui:render()
  eq(lines, api.nvim_buf_get_lines(s.ui.bufs.list, 0, -1, false))
  eq(preview, api.nvim_buf_get_lines(s.ui.bufs.preview, 0, -1, false))
  eq(2, #s:get_selection())
  ...
  ready(s)
  await(function()
    return not s.render_timer
  end)
  eq(1, #s.results)
  eq("src/alpha.lua", s.results[1].text)
  eq(1, s.index)
  for _, count in ipairs(frames) do
    assert(count > 0, "smart rendered an empty list between populated searches")
  end
end)
test("smart replaces retained rows with empty results and errors", function()
  local s = ready(B.smart(opts()))
  s:set_query("absent24122")
  ready(s)
  s.ui:render()
  eq(0, #s.results)
  eq("  No results", api.nvim_buf_get_lines(s.ui.bufs.list, 0, 1, false)[1])
  s:set_query("")
  assert(s.loading, "the empty smart query must wait for fff")
  ready(s)
  eq(2, #s.results)
  s:set_query("file_error")
  await(function()
    return not s.loading and not s.searching and not s.preparing
  end)
  eq(0, #s.results)
  assert(s.error:find("fixture file search failed", 1, true))
  s:set_query("alpah *.lua !test/")
  ready(s)
  eq(1, #s.results)
end)
